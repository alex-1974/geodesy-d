module geodesy.projection.projection_factor_reverse_float_projected_probe;
...
/*
 * FLOAT-R2 research probe.
 *
 * Unlike FLOAT-R1, stdin contains represented projected E/N directly.
 * No geodesy-d forward operation participates in the tested path.
 */
int main(string[] args)
{
    ...
    foreach (line; stdin.byLineCopy())
    {
        ...
        const float easting =
            fields[0].to!float;
        const float northing =
            fields[1].to!float;

        ProjectedCoordinate!float projected;

        if (!ProjectedCoordinate!float.tryFromComponents(
                easting,
                northing,
                projected))
        {
            writefln("PROJECTED_REJECT");
            continue;
        }
