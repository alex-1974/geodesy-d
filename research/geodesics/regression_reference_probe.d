module geodesy.research.regression_reference_probe;
    ...
    pure nothrow @safe @nogc
{
    ...
    return 2.0L * asin(
        clampUnit(sqrt(dx * dx + dy * dy + dz * dz) / 2.0L));
}
...
private Summary validateCorpus(T)(const string dataPath)
{
    ...
    foreach (rawLine; readText(dataPath).splitLines())
    {
        ...
        if (fields.length != 15)
            throw new Exception(
                "unexpected GEO-B regression field count");
        ...
        if (mode == "direct")
        {
            ...
            if (!solver.tryDirect(
                    ...
                    direct))
                throw new Exception(
                    "GEO-B documented direct regression failed");
            ...
            if (!isFinite(positionError) ||
                !isFinite(tangentError))
                throw new Exception(
                    "GEO-B documented direct metric non-finite");
            ...
            if (positionError > LIMIT ||
                tangentError > LIMIT)
                throw new Exception(
                    "GEO-B documented direct regression mismatch");
        ...
        else if (mode == "inverse")
        {
            ...
            if (!solver.tryInverse(start, end, inverse))
                throw new Exception(
                    "GEO-B documented inverse regression failed");
            ...
            if (!isFinite(distanceError))
                throw new Exception(
                    "GEO-B documented inverse distance non-finite");
            ...
            if (distanceError > LIMIT)
                throw new Exception(
                    "GEO-B documented inverse distance mismatch");
            ...
            if (!solver.tryDirect(
                    ...
                    closure))
                throw new Exception(
                    "GEO-B documented inverse closure failed");
            ...
            if (!isFinite(closureError))
                throw new Exception(
                    "GEO-B documented inverse closure non-finite");
            ...
            if (closureError > LIMIT)
                throw new Exception(
                    "GEO-B documented inverse closure mismatch");
        ...
        {
            throw new Exception(
                "unknown GEO-B documented regression mode");
        }
    }
    ...
    const Summary summary)
{
    ...
    writeln("direct regressions: ", summary.directRows);
    writeln("inverse regressions: ", summary.inverseRows);
    writeln(
        "max direct position angle [rad]: ",
        summary.maxDirectPosition);
    writeln(
        "max direct tangent angle [rad]: ",
        summary.maxDirectTangent);
    writeln(
        "max inverse normalized distance error: ",
        summary.maxInverseDistance);
    writeln(
        "max inverse closure angle [rad]: ",
        summary.maxInverseClosure);
}
...
void main(string[] args)
{
    if (args.length != 2)
        throw new Exception(
            "usage: regression_reference_probe " ~
            "<regression-reference.tsv>");

    const doubleSummary = validateCorpus!double(args[1]);
    const realSummary = validateCorpus!real(args[1]);

    assert(doubleSummary.directRows == 2);
    assert(doubleSummary.inverseRows == 9);
    assert(realSummary.directRows == 2);
    assert(realSummary.inverseRows == 9);

    printSummary("double", doubleSummary);
    printSummary("real", realSummary);

    writeln("GEO-B documented regressions: PASS");
}
